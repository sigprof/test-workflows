  ...
  attrsets = {
    /*
     ...
     Parameters:
       ...
       - `recurseCond`: the predicate which determines whether the value
         (which is already verified to be an attribute set) should be recursed
         into.  Takes two arguments: `path` (a list of strings describing the
         path from the root of `nestedAttrs` to the value) and `value` (the
         value at `path` in `nestedAttrs`).
